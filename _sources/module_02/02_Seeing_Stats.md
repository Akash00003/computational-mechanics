...
print("B: It now plots values that were under 0.02 ABV that were not previously shown")
```
...
t2007 = transistor.loc[transistor['Date of Introduction'] == 2007]
#print(t2007['MOS transistor count'])
plt.boxplot(t2007['MOS transistor count'])
Q1_abv = np.percentile(t2007['MOS transistor count'], q=25)
Q2_abv = np.percentile(t2007['MOS transistor count'], q=50)
Q3_abv = np.percentile(t2007['MOS transistor count'], q=75)
...
```{code-cell} ipython3
from matplotlib import cm
doi = transistor['Date of Introduction'].values
mos = transistor['MOS transistor count'].values
plt.figure(figsize=(8,8))
plt.scatter(doi, mos)
plt.title('Scatter plot of Date of Intro vs. MOS Count \n')
plt.xlabel('doi')
plt.ylabel('MOS');
```
